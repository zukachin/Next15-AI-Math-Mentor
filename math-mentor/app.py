...
def _is_api_quota_error(e: Exception) -> Tuple[bool, str]:
    """Returns (is_quota_or_auth_error, provider: 'groq'|'gemini'|'unknown')."""
    err_str = str(e).lower()
    err_type = type(e).__name__
    if "groq" in err_type or "groq" in err_str:
        return True, "groq"
    try:
        import google.api_core.exceptions as gcp
        if isinstance(e, (gcp.ResourceExhausted, gcp.PermissionDenied, gcp.Unauthenticated)):
            return True, "gemini"
    except ImportError:
        pass
    if "429" in err_str or "quota" in err_str or "rate limit" in err_str or "resourceexhausted" in err_str:
        return True, "gemini" if "generativelanguage" in err_str or "gemini" in err_str else "groq"
    if "401" in err_str or "403" in err_str or "invalid" in err_str and "api" in err_str:
        return True, "groq" if "groq" in err_str else "gemini"
...
# API key override for when quota is exceeded
with st.sidebar.expander("🔑 API key override (if quota exceeded)", expanded=False):
    st.caption("Enter a different API key to try when you hit rate limits or run out of quota.")
    override_groq = st.text_input("Groq API key (optional)", type="password", key="override_groq")
    override_gemini = st.text_input("Gemini API key (optional)", type="password", key="override_gemini")
    if st.button("Use these keys"):
        if override_groq:
            os.environ["GROQ_API_KEY"] = override_groq
            st.session_state["api_key_overridden"] = "groq"
        if override_gemini:
            os.environ["GEMINI_API_KEY"] = override_gemini
            st.session_state["api_key_overridden"] = "gemini"
        if override_groq or override_gemini:
            st.success("Keys saved. Try again.")
        else:
            st.warning("Enter at least one key.")
...
if input_mode == "Text":
    ...
    if uploaded_image is not None:
        try:
            ...
            is_err, prov = _is_api_quota_error(e)
            if is_err:
                st.error(f"API quota/auth error ({prov}): {str(e)[:200]}")
                st.info("Open the sidebar → **API key override** to enter a different API key, then try again.")
            else:
                raise
...
if st.button("Solve") and raw_question.strip():
    try:
        ...
        if is_err:
            st.error(f"API quota/auth error ({prov}): {str(e)[:200]}")
            st.info("Open the sidebar → **API key override** to enter a different API key, then click Solve again.")
        else:
            raise
